...
export const generateRenewalLetter = onCall(
  ...
  async (request) => {
    ...
    const { individualId, authorizationId } = (request.data as { individualId: string; authorizationId: string }) ?? {};
    ...
    try {
      ...
      const progressDocs = progressSnap.status === "fulfilled"
        ? (progressSnap.value as admin.firestore.QuerySnapshot).docs
        : [];
      const monitoringDocs = monitoringSnap.status === "fulfilled"
        ? (monitoringSnap.value as any).docs ?? []
        : [];
      const carePlanDocs = carePlanSnap.status === "fulfilled"
        ? (carePlanSnap.value as any).docs ?? []
        : [];
      ...
      const progressSummary = progressDocs.slice(0, 8).map(d => {
        const n = d.data();
        const date = n.date || n.progressDate || n.createdAt?.toDate?.()?.toLocaleDateString?.() || "—";
        const body = (n.detailsOfActivity || n.purposeOfActivity || n.narrative || n.note_body || "").slice(0, 250);
        return `[${date}] ${body}`;
      }).join("\n") || "No recent progress notes on file.";

      const monitoringSummary = monitoringDocs.slice(0, 2).map((d: any) => {
        const n = d.data();
        const actions = (n.recommendedActions || []).slice(0, 3).join("; ");
        return `${n.typeOfReview || "Quarterly"} (${n.completedDate?.toDate?.()?.toLocaleDateString?.() || "—"}): ${actions || n.goalProgress || "No findings."}`;
      }).join("\n") || "No recent monitoring forms on file.";

      const goals = carePlanDocs.length > 0
        ? ((carePlanDocs[0] as any).data().goals || []).slice(0, 4)
          .map((g: any) => `• ${g.title || g.goal}: ${(g.description || "").slice(0, 120)}`)
          .join("\n")
