...
export const dailyAuthRenewalCheck = onSchedule(
  {
  ...
  async (_event) => {
    ...
    for (const authDoc of authsSnap.docs) {
      ...
      created++;
      console.log(
        `[AuthRenewal] Created ${priority} renewal task for auth ${auth.auth_number} (${auth.individualName}, expires ${auth.end_date})`
      );
    }
