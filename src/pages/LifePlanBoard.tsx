...
export default function LifePlanBoard() {
  ...
  const overviewStats = [
    { label: "Total Agents", value: mockAgents.length.toString(), icon: Bot, trend: `${mockAgents.filter((a) => a.status === "active").length} active` },
    { label: "Plans Generated", value: "34", icon: FileCheck, trend: "12 implemented" },
    { label: "Active Users", value: "12", icon: Users, trend: "4 online now" },
    { label: "Success Rate", value: "85%", icon: TrendingUp, trend: "based on implemented" },
  ];
      ...
      <main className="flex-1 overflow-auto p-6">
        <div className="max-w-[1600px] mx-auto">
          ...
          <motion.div
            ...
            className="relative mb-8 p-6 rounded-2xl bg-gradient-to-r from-primary/5 via-primary/10 to-accent/5 border border-primary/10 overflow-hidden"
          >
            ...
            <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl font-display font-bold text-foreground">Lifeplan Board</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            ...
            <div className="flex items-center rounded-lg border border-border p-1">
              <button
                className={cn("p-2 rounded-md transition-colors", viewMode === "grid" ? "bg-secondary" : "hover:bg-secondary/50")}
                onClick={() => setViewMode("grid")}
              >
                <LayoutGrid className="h-4 w-4" />
              </button>
              <button
                className={cn("p-2 rounded-md transition-colors", viewMode === "list" ? "bg-secondary" : "hover:bg-secondary/50")}
              ...
              {filteredAgents.map((agent, i) => (
                ...
                >
                  <LifePlanAgentCard
                    agent={agent}
                    colorIndex={i}
                    onClick={() => navigate(`/lifeplan/agent/${agent.id}`)}
                  />
                </motion.div>
              ))}
            </div>
