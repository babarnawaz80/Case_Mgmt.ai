...
type MatchSignal = "medicaid_id" | "name_dob" | "both";
...
function normalizeMedicaidId(v: string | undefined): string | null {
  if (!v) return null;
...
function namedobKey(first: string, last: string, dob: string | undefined): string | null {
  if (!dob) return null;
  const dobTrimmed = dob.trim();
  if (!dobTrimmed) return null;
  return `${first.trim().toLowerCase()}|${last.trim().toLowerCase()}|${dobTrimmed}`;
}

// ─── Check for existing pair ───────────────────────────────────────────────────

async function pairAlreadyExists(idA: string, idB: string): Promise<boolean> {
...
async function writePair(
  orgId: string,
  indA: IndividualRecord,
  indB: IndividualRecord,
...
async function scanOrgGroup(orgId: string, records: IndividualRecord[]): Promise<void> {
  // Signal A: group by Medicaid ID
  const byMedicaidId = new Map<string, IndividualRecord[]>();
  for (const rec of records) {
    const key = normalizeMedicaidId(rec.medicaid_id);
    if (!key) continue;
    const group = byMedicaidId.get(key) ?? [];
    group.push(rec);
    byMedicaidId.set(key, group);
  }

  // Signal B: group by name+DOB
  const byNameDob = new Map<string, IndividualRecord[]>();
  for (const rec of records) {
    const key = namedobKey(rec.first_name, rec.last_name, rec.dob);
    if (!key) continue;
    const group = byNameDob.get(key) ?? [];
    group.push(rec);
  ...
  const pairSignals = new Map<string, { medicaidId: boolean; nameDob: boolean; recA: IndividualRecord; recB: IndividualRecord }>();
...
async function runDuplicateScan(): Promise<void> {
  ...
  // Group by org
  const byOrg = new Map<string, IndividualRecord[]>();
...
async function runDuplicateScanForNew(
  ...
  for (const docSnap of snap.docs) {
    if (docSnap.id === individualId) continue; // skip self
    const existingData = docSnap.data();
    const existing: IndividualRecord = {
      id: docSnap.id,
      organizationId: orgId,
...
export const detectDuplicatesScheduled = onSchedule(
  {
    schedule: "0 2 * * *",
...
export const detectDuplicatesOnCreate = onDocumentCreated(
...
export const detectDuplicatesOnDemand = onCall(async (_request) => {
  await runDuplicateScan();
  return { success: true };
});
