...
export function InlineIndividualSnapshot({ person }: Props) {
  const navigate = useNavigate();
  const activities = getActivitiesFor(person);
  const alerts = getAlertsFor(person);
  const stats = [
    { label: "Notes (30d)", value: activities.length },
    { label: "Open flags", value: alerts.filter((a) => a.tone !== "info").length },
    { label: "Forms due", value: 2 },
    { label: "Risk", value: person.riskScore ?? "—" },
  ];
    ...
    <div className="space-y-3">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[11px] font-semibold tracking-wide uppercase text-purple-600">
                Individual Snapshot
              </p>
            </div>
            ...
            <p className="text-xs text-muted-foreground">
              {person.age} · {person.gender === "M" ? "Male" : "Female"} · DOB {person.dob} · Updated {person.updatedOn}
            </p>
          </div>
          <Button
            variant="outline"
      ...
      <div className="grid grid-cols-2 gap-3">
          ...
          <dl className="text-xs space-y-1.5">
            <div className="flex justify-between">
              ...
              <dd className="text-foreground font-medium flex items-center gap-1">
                <MapPin className="w-3 h-3" /> {person.county}
              </dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-muted-foreground">Status</dt>
              <dd className="text-foreground font-medium">{person.status}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-muted-foreground">Admitted</dt>
              <dd className="text-foreground font-medium">{person.admittedOn}</dd>
            </div>
            {person.serviceContact && (
              <div className="flex justify-between">
                <dt className="text-muted-foreground">CM</dt>
                <dd className="text-foreground font-medium truncate ml-2">{person.serviceContact}</dd>
              </div>
            )}
          </dl>
        </div>
        ...
        <div className={cn(
          ...
          person.allergies && person.allergies !== "None known"
          ...
          <div className="flex flex-wrap gap-1.5 mb-2">
            {(person.allergies ?? "None known").split(",").map((a) => (
          ...
          {person.specialInstructions && (
            <p className="text-xs text-muted-foreground italic">{person.specialInstructions}</p>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-purple-200 dark:border-purple-500/30 bg-gradient-to-br from-purple-50 to-violet-50/50 dark:from-purple-500/10 dark:to-violet-500/5 p-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-foreground">Convert this snapshot into a note</p>
            <p className="text-xs text-muted-foreground">
              AI will pre-fill from the conversation and last 30 days. You review & apply.
            </p>
          </div>
        </div>
