...
import { progressNotes as seed, ProgressNote } from "@/data/progressNotes";
import { getPerson } from "@/data/people";

const ProgressNoteLog = () => {
  const navigate = useNavigate();
  const [notes, setNotes] = useState<ProgressNote[]>(seed);

  const handleDelete = (id: string) => {
    setNotes((n) => n.filter((x) => x.id !== id));
    toast({ title: "Deleted", description: "Progress note removed." });
  };
                ...
                <tr key={n.id} className="hover:bg-icm-bg/60">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-1">
                      <button
                        onClick={() => navigate(`/people/${n.personId}/progress-note/${n.id}`)}
