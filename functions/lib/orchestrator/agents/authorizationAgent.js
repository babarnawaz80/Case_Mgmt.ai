...
exports.runAuthorizationAgent = runAuthorizationAgent;
const admin = __importStar(require("firebase-admin"));
async function runAuthorizationAgent(individual, runId, orgId, db) {
    var _a, _b;
    const tasks = [];
    const logs = [];
    ...
    try {
        ...
        for (const authDoc of authSnap.docs) {
            ...
            if (!severity)
                continue;
            // ── Create task ────────────────────────────────────────────────────────
            const dueTimestamp = admin.firestore.Timestamp.fromDate(new Date(endDate.getTime() - 14 * 24 * 60 * 60 * 1000));
            tasks.push({
                org_id: orgId,
...
//# sourceMappingURL=authorizationAgent.js.map